...
<!-- <xsl:template match="body/*[position() = 2]" mode="html">
	<xsl:param name="image" />
    <xsl:element name="{.}">
        <xsl:apply-templates mode="html"/>
    </xsl:element>
	<img src="{$workspace}{$image/image/@path}/{$image/image/filename}" alt="" class="full-width" />
	<p class="full-width-caption"><xsl:copy-of select="$image/description"/></p>
</xsl:template> -->
